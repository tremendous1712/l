...
export const SoftmaxView = ({ nextToken }) => {
  ...
  if (!nextToken || !nextToken.probs) {
    return (
      <div style={{ 
        ...
        <div>No softmax data available</div>
        <div style={{ fontSize: '0.8em', marginTop: '20px', color: '#9ca3af' }}>
          {nextToken ? 'NextToken exists but no probs array' : 'NextToken is null/undefined'}
        </div>
        {nextToken && (
          <div style={{ fontSize: '0.8em', marginTop: '10px', color: '#9ca3af' }}>
            NextToken keys: {Object.keys(nextToken).join(', ')}
          </div>
        )}
      </div>
    );
  ...
  // Safety check for empty probs array
  if (!tempProbs || tempProbs.length === 0) {
    return (
      <div style={{ 
        color: '#f87171', 
        textAlign: 'center', 
        ...
        <div>Empty probability data</div>
        <div style={{ fontSize: '0.8em', marginTop: '10px', color: '#9ca3af' }}>
          Probs array length: {nextToken.probs?.length || 0}
        </div>
      </div>
    );
  }
